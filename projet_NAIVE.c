...
double compute(int N, double A[N][N], double conv_mask[3][3], int l, int m, int mat_x_idx, int mat_y_idx)
{
  if (l == 0 && m == 0)
    return A[mat_x_idx-1][mat_y_idx-1]*conv_mask[l][m];
  if (l == 0 && m == 1)
    return A[mat_x_idx-1][mat_y_idx]*conv_mask[l][m];
  if (l == 0 && m == 2)
    return A[mat_x_idx-1][mat_y_idx+1]*conv_mask[l][m];
  if (l == 1 && m == 0)
    return A[mat_x_idx][mat_y_idx-1]*conv_mask[l][m];
  if (l == 1 && m == 1)
    return A[mat_x_idx][mat_y_idx]*conv_mask[l][m];
  if (l == 1 && m == 2)
    return A[mat_x_idx][mat_y_idx+1]*conv_mask[l][m];
  if (l == 2 && m == 0)
    return A[mat_x_idx+1][mat_y_idx-1]*conv_mask[l][m];
      ...
      for (int mat_y_idx = 1; mat_y_idx < N-1; ++mat_y_idx) {
        for (int mask_x_idx = 0; mask_x_idx < 3; ++mask_x_idx) {
          for (int mask_y_idx = 0; mask_y_idx < 3; ++mask_y_idx) {
            out_mat[mat_x_idx][mat_y_idx] +=  (double)6431/N*4 * compute(N, in_mat, conv_mask, mask_x_idx, mask_y_idx, mat_x_idx, mat_y_idx);
          }
        }
      }
    }
